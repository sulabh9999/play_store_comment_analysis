...
class DBWebQuery(DBCore):
    ...
    def __setDefaultQuery(self,
                          start,
                          end,
                          isNegative=True,
                          columnName='rating'):
        return """date BETWEEN '%s' AND '%s' AND %s""" % (
            start, end, self.__setIsNegative(isNegative, columnName))
    ...
    def __getHotTopics(self,
        ...
        parm = (column, column, table,
                self.__setDefaultQuery(start, end, isNegative), column, total)
        # q= """SELECT * FROM """+ table
    ...
    def __getReasonForTopic(self, start, end, topic, total, isNegative=True):
        ...
        parm = (reasonColumn, reasonColumn, table, topicColumn, topic,
                self.__setDefaultQuery(start, end,
                                       isNegative), reasonColumn, total)
    ...
    def getComment(self,
                   ...
                   isNegative=True):
        table = SConstants.table.master
        parm = (table, topic, reason,
                self.__setDefaultQuery(start, end, isNegative), commentsCount)
    ...
    def getChartData(self,
        ...
        finalResult = []
        topicJson = self.__getHotTopics(start, end, 'topic', top, self.table,
                                        isNegative)
        topicDict = json.loads(topicJson)

        for topic, frequency in topicDict:
            reasonList = self.__getReasonForTopic(start, end, topic, 6,
                                                  isNegative)
            ...
            for each in reasonList:
                reason = each[0]
                if len(reason) > 0:
                    commentsJson = self.getComment(start, end, commentsCount,
                                                   topic, reason, isNegative)
...
############################# line chart ######################
    ...
    def __getFreqForReason(self, reason, countTag, dateQuery, country,
                           platform, chunkSize):
        ...
        and reason='service' 
        group by week(date)"""
        parm = (dateColumn, ratingColum, ratingColum, table, dateQuery,
                reasonColumn, reason, dateColumn, dateColumn)
        query = ''
        if chunkSize == 'week':
    ...
    def getLineChart(self, reason, start, end, country, platform, chunkSize):
        ...
        resultJson = self.__getFreqForReason(reason, 'pCount', dateQuery,
                                             country, platform, chunkSize)
