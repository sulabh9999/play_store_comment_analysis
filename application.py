...
application.config.update(JSON_SORT_KEYS=False)
...
def pieChart():
    ...
    if request.method == 'POST':
        ...
        dgStartDate, dgEndDate, dgContry, dgPlatform = getFormInput(
            request.form)
        queryResult = db.getChartData(dgStartDate,
                                      dgEndDate,
                                      dgContry,
                                      dgPlatform,
                                      top=5,
...
def lineChart():
    ...
    if request.method == 'POST':
        ...
        queryResult = db.getLineChart(sliceTitle,
                                      dgStartDate,
                                      dgEndDate,
                                      dgContry,
                                      dgPlatform,
...
def circleChart():
    ...
    if request.method == 'POST':
        ...
        dgStartDate, dgEndDate, dgContry, dgPlatform = getFormInput(req)
        # pieChartTitle, sliceTitle, chunkSize = getPieParmInput(req)
        queryResult = db.getCircleChart(dgStartDate, dgEndDate, dgContry,
                                        dgPlatform)
        return jsonify(queryResult)
